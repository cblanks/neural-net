...
class Neuron():
  ...
  def _setBias(self, bias):
    if bias < 0.0 or bias > 1.0:
      raise ValueError(f'This neuron can have a bias no less than 0.0 and no more than 1.0 but {bias} was received.')
    
    self.bias = bias

  def _setWeights(self, weights):
    if len(weights) < 1:
      raise IndexError(f'This neuron can have must have at least 1 input but {len(weights)} were received.')

    for w in weights:
      if w < 0.0 or w > 1.0:
        raise ValueError(f'This neuron can apply input weights of no less than 0.0 and no more than 1.0 but a weight of {w} was received.')
  ...
  def getOutput(self, input_list):
    if not len(input_list) == self.numberOfInputs():
      raise IndexError(f'This neuron expects {self.numberOfInputs()} inputs but {len(input_list)} were received.')
