...
class NeuronFactory():
    ...
    def combine(self, neuron_list):
        if len(neuron_list) < 2:
            raise IndexError(
                f'This Neuron Factory expects to combine two or more neurons but received {len(neuron_list)}.')
    ...
    def _bias(self, mode=0.5):
        return random.triangular(-1.0, 1.0, mode)

    def _newWeights(self, n, mode=0.5):
        if n < 1:
            raise ValueError(
                f'This Neuron Factory can make neurons with one or more inputs but {n} were requested.')

        new_weights = []
        for i in range(n):
            new_weights.append(random.triangular(-1.0, 1.0, mode))
    ...
    def _combineWeights(self, weights_list):
        for i in range(1, len(weights_list)):
            if not len(weights_list[i]) == len(weights_list[0]):
                raise IndexError(
                    f'This Neuron Factory expects to combine neurons with the same number of inputs but received neurons with {len(weights_list[i])} and {len(weights_list[0])} inputs respectively.')

        new_weights = []
        for i in range(len(weights_list[0])):
            weights = list(map(lambda x: x[i], weights_list))
            avg = self._average(weights)
            new_weights.append(random.triangular(-1.0, 1.0, avg))
...
if __name__ == "__main__":
    ...
    class NeuronFactoryTestMethods(unittest.TestCase):
        ...
        def test_bias_is_combined(self):
            ...
            n2 = f.new(3)
            n3 = f.combine([n1, n2])
            self.assertEqual(n3.bias, -0.23368376071115615)
        ...
        def test_weights_are_combined(self):
            ...
            n2 = f.new(2)
            n3 = f.combine([n1, n2])
            self.assertEqual(n3.input_weights[0], -0.3638237817131279)
            self.assertEqual(n3.input_weights[1], -0.2668561363480412)
